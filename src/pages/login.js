      ...
      if (statusCode === 200) {
        if (version === appVersion) {
          ...
          sessionStorage.setItem(
            "userDetails",
            JSON.stringify({
              MaxisId: state.userName,
              Password: state.password,
            })
          );
          dispatch(setUserInfo({ token: Myosstoken, user: resultData }));
          const { data: menuData } = await getMenuItemsHTTP({
            Action: "Grid",
            LoginUIID: resultData?.LoginUIID,
          });
          ...
          toast.warning(
            "Please clear the browser cache to reflect new changes"
          );
        }
      } else {
