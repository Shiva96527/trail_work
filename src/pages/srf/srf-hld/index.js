...
import {
  ...
  email_log_columns,
  financial_info_columns,
  review_cost_columns_hld,
  workflow_columns,
} from "./config/columns";
...
import {
  ...
  getSrfHLDsHTTP,
  getSrfMailLogsHTTP,
  srfDeleteAttachmentHTTP,
  srfSaveWorkflowHTTP,
  srfUploadAttachmentHTTP,
  updateSrfWorkflowHTTP,
  SrfWMCPQCostUpdateAPI,
} from "../../../services/srf-service";
...
const SRFHLD = () => {
  ...
  const [srfDetails, setSrfDetails] = useState({
    IsChannel: "",
    StatusName: "",
    SRFNumber: "",
    Status: "",
  ...
  const [srfReviewerAdditionalInfoList, setReviewerSRFAdditionalInfoList] =
    useState([]);
  ...
  const getSrfById = async () => {
    ...
    try {
      const {
        data: { data: resultData, statusCode, statusMessage },
      } = await getSrfByIdHTTP(payload);
      if (statusCode === 200) {
        const { IsChannel, StatusName, ServiceType, WFStatusCode, AssignedTo } =
          resultData?.srfActionWorkFlowResponse || {};
        const {
          ExecutiveSummary,
          HighLevelSolution,
          TechnicalRiskAssessment,
        ...
        setSrfDetails({
          ...
          isActionBtnEnableFlag:
            resultData?.srfCreateInfoResponse?.isActionBtnEnableFlag === "Yes"
              ? true
              : false,
        });
        ...
        setReviewerSRFAdditionalInfoList(
          resultData?.srfAdditionalInfoReviewerResponse
        );
  ...
  const getSrfMailLogs = async (integrationId) => {
    ...
    try {
      const {
        data: { data: resultData, statusCode, statusMessage },
      } = await getSrfMailLogsHTTP(payload);
      if (statusCode === 200) {
  ...
  const handleFinancialModal = (data) => {
    const { IntegrationID, SRFNumber, ServiceInfoID, SiteInfoID, SiteSeq } =
      data;
    setSelectedCost({
      IntegrationID,
      SRFNumber,
  ...
  const handleDeleteFile = async (property) => {
    ...
    try {
      ...
      } = await srfDeleteAttachmentHTTP(payload);
      if (statusCode === 200) {
  ...
  const handleFileChange = async (e, property, columnName) => {
    ...
    formData.append(
      "ModuleID",
      localState?.WorkflowId ? localState?.WorkflowId : 0
    );
    ...
    try {
      const {
        data: { data: resultData, statusCode, statusMessage },
      } = await srfUploadAttachmentHTTP(formData);
      if (statusCode === 200) {
        toast.success(statusMessage);
  ...
  const workFlowSave = async (data, action) => {
    ...
    try {
      const {
        data: { statusCode, statusMessage },
      } = await srfSaveWorkflowHTTP(payload);
      if (statusCode === 200) {
        toast.success(statusMessage);
  ...
  const cpqCostRejectHandler = async (data, action) => {
    ...
    try {
      const {
        data: { statusCode, statusMessage },
      } = await SrfWMCPQCostUpdateAPI(payload);
  ...
  const handleReviewerAction = async (rowData, action) => {
    ...
    try {
      const {
        data: { statusCode, statusMessage },
      } = await updateSrfWorkflowHTTP(payload);
      if (statusCode === 200) {
        toast.success(statusMessage);
        getSrfById();
        //navigate(-1);
  ...
  const title = path.includes("inbox")
    ? "Inbox View"
    ...
    <>
      <Card className="card_outer_padding">
        <div className="srfhld-title-container">
          <div style={{ flex: 1 }}>
            {" "}
            {/* This will make the h4 take up remaining space */}
            <span
          ...
          <div className="srfhld-title-pull-right">
            <FontAwesomeIcon
              ...
              className="font-awersome"
              onClick={() => handleExpansion("all")}
            />
            &nbsp;&nbsp;
            <FontAwesomeIcon
              icon={faAngleDoubleUp}
              className="font-awersome"
              onClick={() => handleExpansion("collapse")}
            />
            &nbsp;&nbsp;
        ...
        <CardBody className="accordions-card-body">
          <div>
            <span
              ...
              onClick={() => handleNavigate("/neptune/srf/srfinbox/view")}
            >
              {srfDetails?.SRFNumber}
            </span>
            &nbsp;&nbsp;
            <Badge className="style-cursor" color="primary">
              <span>{`Status->${srfDetails?.StatusName}`}</span>
            </Badge>
            &nbsp;&nbsp;
            <Badge color="success">{`Group->${localState?.GroupName}`}</Badge>
            &nbsp;&nbsp;
            {srfDetails?.AssignedTo && (
              <Badge color="info">
                {"Assigned to->" + srfDetails?.AssignedTo}
              </Badge>
            )}
          </div>
          <br />
          <div className="app-inner-layout__wrapper">
            <Accordion flush open={open} toggle={toggle}>
                ...
                <AccordionItem innerRef={financialRef}>
                  ...
                  <AccordionBody accordionId="1">
                      ...
                      <Row>
                        <Col md={12}>
                          <NeptuneAgGrid
                            refId={"srf-financial"}
                            data={financialList}
                            dataprops={financial_info_columns(
                              handleFinancialModal,
                              ismpnService
                            )}
                            paginated={false}
                            itemsPerPage={10}
                    ...
                    <fieldset disabled={hideActions}>
                      {showFinancialModal && (
                        <SrfFinancialModal
                          status={srfDetails?.StatusName}
                          hideActions={hideActions}
                          isOpen={showFinancialModal}
                          onClose={() => setShowFinancialModal(false)}
                          selectedCost={selectedCost}
                          getAllCost={gerSrfHldGrid}
                        />
                      )}
                    </fieldset>
              ...
              {(srfDetails?.StatusName === "HLD" ||
                srfDetails?.StatusName === "Closed" ||
                (location.pathname.includes("outbox") &&
                  srfDetails?.StatusName === "HLD (Pending)")) && (
                <AccordionItem>
                        ...
                        <Row>
                          <Col md={12}>
                            <NeptuneAgGrid
                              ...
                              dataprops={review_cost_columns_hld(
                                handleReviewCostRowClick,
                                handleReviewerAction,
                                !hideActions ||
                                  location.pathname.includes("outbox")
                              )} // && srfDetails?.StatusName !== 'HLD' this condition is not needed for hld case
                              paginated={false}
                              itemsPerPage={10}
                              searchable={false}
                              exportable={false}
                          ...
                          <Row>
                            ...
                            <Col md={6}>
                              <FormInput
                                ...
                                control={control}
                                rules={{
                                  required: "High Level Solution is required",
                                }}
                                errors={errors}
                              />
                            </Col>
                              ...
                              {!getValues("hldFile") ? (
                                ...
                                <>
                                  <span
                                    className="link-style"
                                    onClick={() =>
                                      handleViewFile(getValues("hldFile"))
                                    }
                                  >
                                    {getValues("hldFile")?.FileName}
                                  </span>
                                  <FontAwesomeIcon
                                    icon={faTrash}
                                    color="red"
                                    onClick={() => handleDeleteFile("hldFile")}
                                    fontSize={"12px"}
                                    cursor={"pointer"}
                                  />
                                </>
                              ...
                              <FormGroup>
                                {!getValues("financialFile") ? (
                                  ...
                                  <>
                                    ...
                                    >
                                      {getValues("financialFile")?.FileName}
                                    </span>
                                    <FontAwesomeIcon
                                      icon={faTrash}
                                      color="red"
                                      onClick={() =>
                                        handleDeleteFile("financialFile")
                                      }
                                      fontSize={"12px"}
                                      cursor={"pointer"}
                                    />
                        ...
                        <Row>
                          <Col md={12}>
                            <NeptuneAgGrid
                              ...
                              dataprops={additional_info_columns(
                                handleDownloadFile
                              )}
                              paginated={false}
                              itemsPerPage={10}
                          ...
                          <div>
                            {srfDetails?.StatusName === "HLD Cost Pending" &&
                              srfDetails?.StatusName !== "Closed" && (
                                <Button
                                  color="primary"
                                  onClick={() =>
                                    handleCustomSubmit("Save Additional Info")
                                  }
                                >
                            ...
                            {srfDetails?.StatusName === "HLD Cost Pending" && (
                              <Button
                                color="primary"
                                onClick={() =>
                                  handleCustomSubmit("Submit for Review")
                                }
                              >
                                Submit for Review
                              </Button>
                            ...
                            {srfDetails?.StatusName === "HLD Cost Pending" && (
                              <Button
                                color="danger"
                                onClick={() =>
                                  handleCustomSubmit("Not Involved")
                                }
                              >
                                Not Involved
                              </Button>
                            ...
                            {srfDetails?.StatusName === "HLD" &&
                              srfDetails?.IsChannel === "CPQ" && (
                                <Button
                                  color="primary"
                                  onClick={() =>
                                    handleCustomSubmit(
                                      "Update Cost and Close SRF"
                                    )
                                  }
                                >
                                  Submit Cost to CPQ & Close SRF
                                </Button>
                              )}
                            {srfDetails?.StatusName === "HLD" &&
                              srfDetails?.IsChannel === "Neptune" && (
                                <Button
                                  color="primary"
                                  onClick={() =>
                                    handleCustomSubmit(
                                      "Manual Update Cost and Close SRF"
                                    )
                                  }
                                >
                                  Submit Cost & Close SRF
                                </Button>
                              )}
                          </div>
                        )}
                    ...
                    <Row>
                      <Col md={12}>
                        <FormGroup>
                          ...
                          />
                          {remarksError && (
                            <span className="required">{remarksError}</span>
                          )}
                        </FormGroup>
                      </Col>
                    </Row>
                      ...
                      <div>
                        {
                          <Button
                            color="danger"
                            onClick={() =>
                              cpqCostRejectHandler(getValues(), "Reject to CPQ")
                            }
                          >
                            Reject to CPQ
                          </Button>
      ...
      {showReviewCostModal && (
        <ReviewCostModal
          ...
          state={state}
          reviewCostData={hldReviewCostList}
          isOpen={showReviewCostModal}
          onClose={handleReviewCloseModal}
          onSuccess={handleGroupAddition}
        />
      )}
    </>
  );
