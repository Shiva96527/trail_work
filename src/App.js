...
import { setMenuItems, setSrfDropdownOptions, setUserInfo } from './redux/slices/globalSlice';
import { getDropdownByTypeHTTP } from './services/global-service';

function App() {
  const dispatch = useDispatch();
  const { userInfo = null } = useSelector(state => state?.globalSlice);
  const router = useRoutes(approutes(sessionStorage.getItem('userInfo') || userInfo?.user?.User_ID));
  ...
  const getDropdownValues = async () => {
    try {
      const { data: { data: resultData, statusCode } } = await getDropdownByTypeHTTP({ DropDownType: "SRF DropDown Values", LoginUIID: sessionStorage.getItem('uiid') });
      if (statusCode === 200) {
        const dropdowns = resultData?.reduce((acc, data) => {
          acc[data?.DropDownType] = { DDId: data?.DDId, dropdownType: data?.DropDownType, dropdownValue: data?.DropDownValue?.split(',') };
          return acc;
        }, {});
